...
The console listens to user input to generate a command. These commands can be 'open', 'close', 'relax', 'force $<$ target force $>$', 'get\-\_\-force' and 'exit' The 'open' command manually opens the gripper. The 'close' command manually closes the gripper. The 'relax' command relaxes the gripper (e.\-g. set the target force to 0). This can only be done manually. The 'force $<$ target force $>$' command sets the target force to the '$<$ target force $>$' parameter. The 'get\-\_\-force' command returns the target force as has been set. The 'exit' command passes on the shutdown command to all listening nodes before shutting down itself. This should be used to control all nodes.
